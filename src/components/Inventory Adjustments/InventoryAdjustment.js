...
import { Label, TextInput, Button,Dropdown, Select,Drawer } from 'flowbite-react';
...
const [editingId, setEditingId] = useState(null);
 ...
 const {token}=useAuth();
  ...
  const fetchAdjustments = async () => {
    setLoading(true);
    try {
      const res = await axios.get('http://43.250.40.133:5005/api/v1/inventory-adjustments',{
          headers: {
...
const fetchStatistics = async () => {
  try {
    const res = await axios.get('http://43.250.40.133:5005/api/v1/inventory-adjustments/statistics', {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
...
const handleExportToExcel = () => {
  const exportData = adjustments.map(({ 
    product, batchNumber, location, adjustmentType, quantity, 
    reason, reasonCategory, status, createdAt 
  }) => ({
    Product: product._id,
  ...
  try {
    ...
    toast.error(isEditing ? "Failed to update adjustment." : "Failed to add adjustment.");
  } finally {
    setLoading(false);
  }
};
...
<div className="flex items-center justify-between flex-wrap px-4 py-2">
  ...
  <div className="flex-1 text-center ml-[-60px]"> 
    <h2 className="font-bold text-lg">INVENTORY ADJUSTMENTS</h2> {/* Reduced size */}
  </div>
...
<Drawer open={openStatisticsDrawer} onClose={() => setOpenStatisticsDrawer(false)} position="right" className="w-[80vw] max-w-[40rem]">
  <div className="p-4">
  ...
  <div className="bg-gray-100 p-4 rounded">
    <strong>Total Adjustments:</strong> {statistics?.totalAdjustments ?? 0}
  </div>
  <div className="bg-gray-100 p-4 rounded">
    <strong>Financial Impact:</strong> ₹{statistics?.totalFinancialImpact ?? 0}
  </div>
  <div className="bg-gray-100 p-4 rounded">
    <strong>Damage Adjustments:</strong> {statistics?.damageAdjustments ?? 0}
  </div>
  <div className="bg-gray-100 p-4 rounded">
    <strong>Missing Adjustments:</strong> {statistics?.missingAdjustments ?? 0}
  </div>
  <div className="bg-gray-100 p-4 rounded">
    <strong>Return Adjustments:</strong> {statistics?.returnAdjustments ?? 0}
  </div>
  <div className="bg-gray-100 p-4 rounded">
    <strong>Processed Adjustments:</strong> {statistics?.processedAdjustments ?? 0}
  </div>
  <div className="bg-gray-100 p-4 rounded">
    <strong>Pending Adjustments:</strong> {statistics?.pendingAdjustments ?? 0}
  </div>
  <div className="bg-gray-100 p-4 rounded col-span-2">
    <strong>Date Range:</strong> {new Date(statistics?.dateRange?.startDate).toLocaleDateString()} - {new Date(statistics?.dateRange?.endDate).toLocaleDateString()}
  </div>
</div>
      ...
      <Drawer open={openModal} onClose={onCloseModal} className="w-[90vw] max-w-[45vw]" position="right">
        <div className="p-4">
          <div className="flex justify-between items-center border-b pb-4">
            ...
            {isEditing ? "Edit Inventory Adjustment" : "Add Inventory Adjustment"}
           </h2>
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
  ...
  <div>
    <Label htmlFor="batchNumber" className="mb-1 block">Batch Number</Label>
    <TextInput
      id="batchNumber"
      name="batchNumber"
  ...
  <div>
    ...
    <div>
  <Label htmlFor="adjustmentType" className="mb-1 block">Adjustment Type</Label>
  <Select
    id="adjustmentType"
    name="adjustmentType"
    className='w-56'
  ...
  <div>
   ...
   <div>
  <Label htmlFor="reasonCategory" className="mb-1 block">Reason Category</Label>
  <Select
    id="reasonCategory"
    name="reasonCategory"
    className='w-56'
